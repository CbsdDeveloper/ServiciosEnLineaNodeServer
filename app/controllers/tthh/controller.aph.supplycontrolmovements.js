...
module.exports = {
	...
	async insertInventory(req, res, next){
		...
		try {
			...
			if( parseInt(inventoryData.movimiento_cantidad)<0 ){
				...
				let currentInventory = await seq.query("SELECT * FROM tthh.vw_stock_inventarioaph WHERE fk_inventario_id = :inventoryId AND fk_insumo_id = :supplyId", replacements);
			...
			let supplyStock = await seq.query("SELECT * FROM tthh.vw_stock_insumosaph WHERE insumoId = :supplyId", replacements);
	...
	async suppliesByInventoryId(req, res){
		...
		let stockList = await seq.query("SELECT * FROM tthh.vw_stock_insumosaph", replacements);
		stockList.forEach((v, k) => { auxModel[v.insumoid] = parseInt(v.stocktotal); });

		let suppliesMovements = await seq.query("SELECT * FROM tthh.vw_stock_inventarioaph WHERE fk_inventario_id = :inventoryId", replacements);
